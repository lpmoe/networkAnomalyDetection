...
file.names <- list.files(getwd(), pattern="SpecificPortbatch*", all.files=FALSE, full.names=FALSE)
length(file.names)
file.names
test <- NULL
...
nrow(test2)
head(test2)
names(test2) <- c("timeoriginal", "data")
...
str(res2)
write.csv(res2, file = "specificport10dayshourly.csv")
#str(res3)
